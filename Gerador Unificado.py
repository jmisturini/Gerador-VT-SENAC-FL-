...
workbook_request = load_workbook(filepath + "\\Desktop\\" + "Gerador\\Pedido de Compra.xlsx", data_only=True)
...
for worksheet_request in workbook_request.worksheets[0:2]:
    ...
    if user_input.lower() == "s":
        if worksheet_request.title == "VT COLABORADORES":
            workbook_base = load_workbook(filepath + "\\Desktop\\" + "Gerador\\Planilha_Base.xlsx")
            ...
            workbook_base.save(filepath + "\\Desktop\\" + "Gerador\\Gerados\\Tabela Vale Transporte Colaboradores" + "(" + str_current_datetime + ")" + ".xlsx")
    ...
    if worksheet_request.title == "VT PROFESSORES":
        ...
        for x in range (len(ids)):
            ...
            worksheet_base.cell(row=x+5,column=3).value=totals[x]
        print ("Gerando Planilha")
        workbook_base.save(filepath + "\\Desktop\\" + "Gerador\\Gerados\\Tabela Vale Transporte Professores" + "(" + str_current_datetime + ")" + ".xlsx")
...
with open (filepath + "\\Desktop\\" + "Gerador\\Gerados\\Quantidade Passes.txt", "w") as file:
